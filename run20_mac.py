...
programma = str(sys.argv[1]) + '\n'
doolhof = str(sys.argv[2])  
kostenKaart = ast.literal_eval(str(sys.argv[3]))
...
def verwerkStapVooruit(status):
    ...
    if status.cells[waarheen[0]][waarheen[1]][0].lower() in 'csbder':
        ...
        logItem ("kosten voor stap vooruit: " + str(status.kostenKaart['stapAchteruit']) + " naar: [" + str(waarheen[0]) +"," + str(waarheen[1]) + "]")
